...
class SimpleSpatialAnalyzer {
  ...
  async loadData() {
    ...
    try {
      ...
      console.log(
        `✓ Loaded ${this.buildings.features.length} buildings and ${this.shelters.features.length} shelters`
      );
  ...
  async loadPolygonLayers() {
    ...
    try {
      // Set up statistical areas for lazy loading (no immediate download)
      console.log("📊 Setting up statistical areas...");
      this.statisticalAreas = []; // Will be loaded on-demand
      console.log("✓ Statistical areas configured for lazy loading");
  ...
  async loadHabitationClustersGeoJson() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          `✓ Loaded ${this.habitationClusters.length} habitation clusters`
        );
      }
    } catch (error) {
  ...
  async loadStatisticalAreasGeoJson() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          `✓ Loaded ${this.statisticalAreas.length} statistical areas`
        );
      }
    } catch (error) {
  ...
  async getOptimalLocations(numShelters) {
    ...
    const maxShelters = Math.min(
      numShelters,
      this.MAX_SHELTERS,
      data.optimal_locations.length
    );
  ...
  getRequestedShelterEvaluation(numNewShelters = 0) {
    ...
    if (!data || !data.existing_shelters || !data.optimal_locations)
      return null;

    // Get requested shelters (status "Request")
    const requestedShelters = data.existing_shelters.filter(
      (shelter) => shelter.properties && shelter.properties.status === "Request"
    );
    ...
    const sortedRequested = [...requestedShelters].sort(
    ...
    const totalImprovement = pairedShelters.reduce(
      (sum, pair) => sum + pair.improvement,
      0
    );
  ...
  calculateCoverageStats(proposedShelters = null) {
    ...
    const newPeopleCovered = selectedOptimal.reduce(
      (sum, loc) => sum + (loc.buildings_covered || 0) * 7,
      0
    ); // Use consistent 7 people per building calculation
    const newBuildingsCovered = selectedOptimal.reduce(
      (sum, loc) => sum + (loc.buildings_covered || 0),
      0
    );
    ...
    return {
      ...
      coveragePercent:
        stats.total_people > 0 ? (totalCoverage / stats.total_people) * 100 : 0,
    };
  }
  ...
  getActiveShelters() {
    ...
    return this.shelters.features.filter((shelter) => {
      ...
      if (this.includeRequestedShelters) {
        ...
        return (
          shelter.properties.status === "Built" ||
          shelter.properties.status === "Request"
        );
      } else {
        // Only built shelters
  ...
  getRequestedShelters() {
    ...
    return this.shelters.features.filter(
      (shelter) => shelter.properties && shelter.properties.status === "Request"
    );
  }
  ...
  getHabitationClusters() {
    return {
      ...
      features: this.habitationClusters || [],
    };
  }
}
